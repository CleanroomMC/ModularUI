...
public abstract class WorldSceneRenderer {
    ...
    public void render(float x, float y, float width, float height, int mouseX, int mouseY) {
        // setupCamera
        Rectangle positionedRect = getPositionedRect((int) x, (int) y, (int) width, (int) height);
        Rectangle mouse = getPositionedRect(mouseX, mouseY, 0, 0);
        mouseX = mouse.x;
        mouseY = mouse.y;
        ...
        if (onLookingAt != null && mouseX > positionedRect.x &&
                mouseX < positionedRect.x + positionedRect.width && mouseY > positionedRect.y &&
                mouseY < positionedRect.y + positionedRect.height) {
    ...
    public void setCameraLookAt(Vector3f lookAt, double radius, double rotationPitch, double rotationYaw) {
        ...
        this.eyePos = pos.translate(lookAt.x, lookAt.y, lookAt.z);
    }

    protected Rectangle getPositionedRect(int x, int y, int width, int height) {
        return new Rectangle(x, y, width, height);
    }

    protected void setupCamera(Rectangle rect) {
        int x = rect.x;
        int y = rect.y;

...
public class MapBasedOperatorDictionary implements OperatorDictionaryIfc {
    ...
    public Set<String> getAvailablePrefixOperatorNames() {
        return Collections.unmodifiableSet(prefixOperators.keySet());
    }

    @Override
    public Set<String> getAvailablePostfixOperatorNames() {
        return Collections.unmodifiableSet(postfixOperators.keySet());
    }

    @Override
    public Set<String> getAvailableInfixOperatorNames() {
        return Collections.unmodifiableSet(infixOperators.keySet());
    }

    @Override
    public Set<OperatorIfc> getAvailablePrefixOperators() {
        return new ObjectOpenHashSet<>(prefixOperators.values());
    }

    @Override
    public Set<OperatorIfc> getAvailablePostfixOperators() {
        return new ObjectOpenHashSet<>(postfixOperators.values());
    }

    @Override
    public Set<OperatorIfc> getAvailableInfixOperators() {
        return new ObjectOpenHashSet<>(infixOperators.values());
    }
}

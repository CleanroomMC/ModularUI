...
public class DateTimeNewFunction extends AbstractFunction {
    ...
    public void validatePreEvaluation(Token token, EvaluationValue... parameterValues)
        ...
        if (parameterValues[parameterLength - 1].isStringValue()) {
            if (!ArrayUtils.contains(getTimeZones(), parameterValues[parameterLength - 1].getStringValue())) {
                throw new EvaluationException(token, "Time zone with id '"
                        + parameterValues[parameterLength - 1].getStringValue() + "' not found");
            }
            parameterLength--;
        }

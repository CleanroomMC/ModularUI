...
println(result.getNumberValue()); // prints 1.25
```
...
println(result.getNumberValue()); // prints 6.00
```
...
println(result.getNumberValue()); // prints 3
        System.out.

println(copiedResult.getNumberValue()); // prints 7
```
...
println(result.getStringValue()); // prints "true : 24.7"
```
...
println(result.getBooleanValue()); // prints true
```
...
getStringValue()); // prints Hello Frank, you are 38
```
...
println(result.getNumberValue()); // prints 4
```
...
put("price",new BigDecimal("14.95"));

        order.

put("positions",List.of(position));
...
println(result); // will print "EvaluationValue(value=PT12H4M30S, dataType=DURATION)"
```
